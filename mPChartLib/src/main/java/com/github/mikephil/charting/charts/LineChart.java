		...
		for (int i = 0; i < mData.getDataSetCount(); i++) {
			...
			if (dataSet.isDrawCubicEnabled()) {
					...
					for (int j = 0; j < points.size() * mPhaseX; j++) {
						...
						if (j == 0) {
							...
							spline.cubicTo(prev.x + prev.dx, (prev.y + prev.dy)
									* mPhaseY, point.x - point.dx,
									(point.y - point.dy) * mPhaseY, point.x,
									point.y * mPhaseY);
						}
					}
				}
				...
				if (dataSet.isDrawFilledEnabled()) {
					...
					float fillMin = mFillFormatter.getFillLinePosition(dataSet,
							mData, mYChartMax, mYChartMin);
				...
				if (dataSet.getColors() == null
					...
					for (int j = 0; j < (valuePoints.length - 2) * mPhaseX; j += 2) {
						...
						mDrawCanvas.drawLine(valuePoints[j],
								valuePoints[j + 1], valuePoints[j + 2],
				...
				if (dataSet.isDrawFilledEnabled() && entries.size() > 0) {
					...
					Path filled = generateFilledPath(entries,
							mFillFormatter.getFillLinePosition(dataSet, mData,
				...
				for (int j = 0; j < positions.length * mPhaseX; j += 2) {
					...
					if (mDrawUnitInChart) {
						...
						mDrawCanvas.drawText(
								mUnit + mValueFormatter.getFormattedValue(val),
